...
internal fun stopMatchesText(stop: StopEntity, inputText: String): Boolean {
...
internal fun findStopByAnyName(stops: List<StopEntity>, inputText: String): StopEntity? {
    val normalized = inputText.trim()
...
internal fun migrateLegacySavedRoutes(
    ...
    return legacyRoutes.mapNotNull { legacy ->
...
internal fun resolveSavedRouteLabel(
    ...
    val fromName = stopMap[route.fromStopId]?.getNameForLanguage(language)
        ?: route.fromFallbackName.ifBlank { route.fromStopId.toString() }
    val toName = stopMap[route.toStopId]?.getNameForLanguage(language)
